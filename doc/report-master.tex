...
La infraestructura obtuvo correctamente información de {\it hardware} del sistema incluyendo cantidad de memoria disponible (9784 MiB), modelo de la unidad de procesamiento (i5-320M @ 2.6 GHz), modelo del controlador de la interfaz de red (82540EM) y modelo de placa madre más sus componentes (440FX). Las especificaciones listadas en la Figura \ref{fig:hw} de memoria, procesador, placa madre y almacenamiento pueden ser utilizadas para buscar información de rendimiento publicada oficialmente por cada fabricante.
...
De esta información podemos conocer por ejemplo, que el procesador utilizado posee 2 cores permitiendo 4 hilos corriendo en paralelo, con frecuencia base de 2.6 GHz y máxima de 3.3 GHz según \href{http://ark.intel.com/products/64896/Intel-Core-i5-3320M-Processor-3M-Cache-up-to-3\_30-GHz}{Automated Relational Knowledgebase} (ARK).

\bigskip

La infraestructura informó correctamente un núcleo {\it Linux} versión 3.16.0 en un sistema {\it Ubuntu} versión 14.04, incluyendo un compilador {\it GCC} versión 4.8.2 y una librería {\it GNU C} versión 2.19. La identificación de la Figura \ref{fig:sw} del {\it software} utilizado en el sistema permite ser más específicos al comparar los resultados obtenidos.
...
La infraestructura ejecutó exitosamente pruebas de rendimiento donde entre otras se incluyen {\it DGEMM} (con 1.8 mflops) y {\it STREAM} (con 7 MB/s). Estos datos sirven como límite de capacidad del sistema en la práctica ya que son núcleos de cómputo denso diseñados para obtener rendimiento en bruto.

\begin{center}
\fbox{\includegraphics[width=6cm]{sp-02.png}}
\end{center}

\section{Código de Prueba}

Esta sección incluye una revisión de los resultados específicos obtenidos al aplicar la infraestructura a códigos de prueba. Se seleccionaron implementaciones de núcleos de cómputo simples pero suficientemente interesantes para demostrar las capacidades de la infraestructura. Entre ellos una implementación directa (no optimizada) de multiplicación de matrices, una simulación de transferencia de calor con un componente de ejecución serial significativo, y por último una implementación de cálculo de fractales sobre números complejos.
...
\item El caso de prueba es muy estable, con una desviación estandar mucho menor a 1 como puede verse en la Figura \ref{fig:est}. Esto implica que el tamaño de los datos de entrada son apropiados para realizar un análisis de rendimiento sin introducir datos aleatorios de ejecución. 

\begin{figure}[H]
\fbox{\includegraphics[width=4cm]{mm-01.png}}
\fbox{\includegraphics[width=6cm]{mm-02.png}}
\caption{Estabilidad de la Multiplicación de Matrices}
\label{fig:est}
\end{figure}

\item Las optimizaciones que realiza el compilador demuestran un impacto importante en el tiempo de ejecución, como puede verse en la Figura \ref{fig:comp}. Esto implica que las optimizaciones automáticas realizadas por el compilador encuentran en el código oportunidades claras de optimización.

\begin{figure}[H]
\begin{center}
\fbox{\includegraphics[width=10cm]{mm-03.png}}
\caption{Optimización en la Multiplicación de Matrices}
\label{fig:comp}
\end{center}
\end{figure}

\item Los tiempos de ejecución al escalar el problema descubren un pico que posiblemente implique un error de implementación de algoritmos, de igual modo los tiempos de ejecución al incrementar las unidades de cómputo revelan un problema ya que no muestra mejoras constantes. Como puede verse en Figuras \ref{fig:esc1} y \ref{fig:esc2}. Esto implica que la implementación del código no está optimizada para los recursos de cómputo disponibles. En este caso es recomendable la revisión de publicaciones en el tema y la reutilización de librerías ya optimizadas.
  
\begin{figure}[H]
\begin{center}
\fbox{\includegraphics[width=10cm]{mm-04.png}}
\caption{Escalamiento del Problema en la Multiplicación de Matrices}
\label{fig:esc1}
\end{center}
\end{figure}
...
\begin{figure}[H]
\begin{center}
\fbox{\includegraphics[width=8cm]{mm-05.png}}
\caption{Escalamiento del Cómputo en la Multiplicación de Matrices}
\label{fig:esc2}
\end{center}
\end{figure}

\item En la Figura \ref{fig:par} la infraestructura estima un 85\% de paralelismo, con una mejora teórica máxima de 7 veces sin escalar el tamaño del problema si utilizamos el mismo algoritmo.

\begin{figure}[H]
\begin{center}
\fbox{\includegraphics[width=8cm]{mm-06.png}}
\caption{Paralelismo en la Multiplicación de Matrices}
\label{fig:par}
\end{center}
\end{figure}

\item El cuello de botella se localiza en el código que obtiene el valor parcial de un elemento de la matriz resultante. Esto identifica la operación a optimizar para maximizar la ganancia en la Figura \ref{fig:bot}.

\begin{figure}[H]
\begin{center}
\fbox{\includegraphics[width=10cm]{mm-07.png}}
\caption{Cuello de Botella en la Multiplicación de Matrices}
\label{fig:bot}
\end{center}
\end{figure}
 
\item La Figura \ref{fig:res} muestra que no hay escrituras al disco, por lo que no se está perdiendo rendimiento al utilizar memoria virtual. Esto implica que el tamaño elegido de problema puede ser cálculado integramente utilizando la memoria principal.

\begin{figure}[H]
\begin{center}
\fbox{\includegraphics[width=10cm]{mm-08.png}}
\caption{Escritura a Disco en la Multiplicación de Matrices}
\label{fig:res}
\end{center}
\end{figure}

\item El cuello de botella principal identificado en la Figura \ref{fig:wr} utiliza la instrucción {\tt add}, la cual no es vectorial. Esto implica que el código no está siendo totalmente vectorizado, desaprovechando la capacidad de cómputo disponible.

\begin{figure}[H]
\begin{center}
\fbox{\includegraphics[width=8cm]{mm-09.png}}
\caption{Instrucciones en la Multiplicación de Matrices}
\label{fig:wr}
\end{center}
\end{figure}

\item Las vectorizaciones no son realizadas exitosamente por el compilador en ningún ciclo dentro del código (Figura \ref{fig:vec}). Esto implica que los ciclos de cómputos del algoritmo no están siendo vectorizados, desaprovechando la capacidad de cómputo disponible. Una comparación de rendimiento de diferentes algoritmos de multiplicación de matrices puede encontrarse en \cite{mm-tool}.

\begin{figure}[H]
\begin{center}
\fbox{\includegraphics[width=10cm]{mm-10.png}}
\caption{Vectorización en la Multiplicación de Matrices}
\label{fig:vec}
\end{center}
\end{figure}
...
Se utiliza una grilla donde cada celda transfiere calor a sus vecinos en una serie finita de ciclos simulando el paso del tiempo basado en el curso \href{http://www.rblasch.org/studies/cs580/pa5/\#Source+Code-N100AC}{CS580} de {\it Ronald Blasche}. El código utilizado es incluido a continuación en el listado \ref{lst:heat}, la configuración del caso de prueba en el listado \ref{lst:cfg-heat}.
...
El reporte generado por la infraestructura puede consultarse en el apéndice \ref{example-heat2d}. Algunas conclusiones que pueden obtenerse al revisar la información generada son:

\begin{enumerate}
\item El caso de prueba es estable ya que la Figura \ref{fig:est2} muestra una desviación baja y el histograma de ejecuciones muestra agrupaciones.

\begin{figure}[H]  
\fbox{\includegraphics[width=4cm]{he-01.png}}
\fbox{\includegraphics[width=7cm]{he-02.png}}
\caption{Estabilidad en la Transferencia de Calor}
\label{fig:est2}
\end{figure}

\item Las optimizaciones del compilador tienen impacto acotado según demuestra la Figura \ref{fig:opt2}.

\begin{figure}[H]  
\begin{center}
\fbox{\includegraphics[width=10cm]{he-03.png}}
\caption{Optimizaciones en la Transferencia de Calor}
\label{fig:opt2}
\end{center}
\end{figure}

\item El tiempo de ejecución al incrementar el tamaño del problema no crece monotónicamente, según lo muestra la Figura \ref{fig:prob2}.

\begin{figure}[H]  
\begin{center}
\fbox{\includegraphics[width=10cm]{he-04.png}}
\caption{Escalamiento del Problema en la Transferencia de Calor}
\label{fig:prob2}
\end{center}
\end{figure}
  
\item El tiempo de ejecución al sumar más unidades de cómputo no decrece sistemáticamente como muestra la Figura \ref{fig:comp2}.

\begin{figure}[H]  
\begin{center}
\fbox{\includegraphics[width=10cm]{he-05.png}}
\caption{Escalamiento del Cómputo en la Transferencia de Calor}
\label{fig:comp2}
\end{center}
\end{figure}
  
\item El paralelismo en el programa solo alcanza el 65\%, por lo tanto el límite máximo de mejora es de 3 veces como lo muestra la Figura \ref{fig:paral2}.

\begin{figure}[H]  
\begin{center}
\fbox{\includegraphics[width=8cm]{he-06.png}}
\caption{Paralelismo en la Transferencia de Calor}
\label{fig:paral2}
\end{center}
\end{figure}
  
\item Hay dos cuellos de botella con un 25\% y un 15\% del tiempo de ejecución, como demuestra la Figura \ref{fig:bot2}.

\begin{figure}[H]
\begin{center}
\fbox{\includegraphics[width=8cm]{he-07.png}}
\caption{Escalamiento del Problema en la Transferencia de Calor}
\label{fig:bot2}
\end{center}
\end{figure}

\item La utilización del CPU no es uniforme, demostrado en la Figura \ref{fig:cpu2}.

\begin{figure}[H]  
\begin{center}
\fbox{\includegraphics[width=10cm]{he-08.png}}
\caption{Utilización del Procesador en la Transferencia de Calor}
\label{fig:cpu2}
\end{center}
\end{figure}
  
\item El 35\% del tiempo de ejecución lo ocupa la instrucción {\tt mov} según la Figura \ref{fig:ins2}. Esto refleja que la implementación invierte tiempo en mover datos en memoria y no tanto en cómputo puro.
  
\begin{figure}[H]
\begin{center}
\fbox{\includegraphics[width=10cm]{he-09.png}}
\caption{Instrucciones en la Transferencia de Calor}
\label{fig:ins2}
\end{center}
\end{figure}
  
\item Los ciclos no están siendo vectorizados de acuerdo a la salida del compilador como lo muestra la Figura \ref{fig:vec2}.

\begin{figure}[H]
\begin{center}
\fbox{\includegraphics[width=10cm]{he-10.png}}
\caption{Vectorización en la Transferencia de Calor}
\label{fig:vec2}
\end{center}
\end{figure}
...
El código utilizado se incluye a continuación en el listado \ref{lst:mandel}, la configuración utilizada para el caso de prueba en el listado \ref{lst:cfg-mandel}. El reporte generado puede consultarse en el apéndice \ref{example-mandel}.
...
Como en los casos anteriores, la configuración refleja que el programa toma un parámetro de una variable de ambiente {\tt N} y al implementar extensiones {\it OpenMP} se puede utilizar {\tt OMP\_NUM\_THREADS} para controlar la cantidad de hilos de ejecución dedicados. Además, la compilación se realiza con {\it GNU Make} y se seleccionaron 8 ejecuciones como las repeticiones deseadas para promediar resultados.
...
Algunas conclusiones que pueden obtenerse al analizar la información generada con la infraestructura desarrollada son:

\begin{enumerate}
\item La estructura utilizada para representar números complejos está alineada y no posee huecos según la Figura \ref{fig:hole3}. Esto garantiza una buena utilización de la jerarquía de memoria.

\begin{figure}[H]
\begin{center}
\fbox{\includegraphics[width=7cm]{mb-00.png}}
\end{center}
\caption{Estructuras en Conjuntos de {\it Mandelbrot}}
\label{fig:hole3}
\end{figure}

\item El caso de prueba es muy estable como lo demuestra la figura \ref{fig:est3}.

\begin{figure}[H]
\fbox{\includegraphics[width=4cm]{mb-01.png}}
\fbox{\includegraphics[width=6cm]{mb-02.png}}
\caption{Estabilidad en Conjuntos de {\it Mandelbrot}}
\label{fig:est3}
\end{figure}
  
\item La proporción del trabajo en paralelo es demasiado alta e incluso mejor que un caso ideal según la Figura \ref{fig:par3}. Lo cual implica que hay problemas de implementación.
  
\begin{figure}[H]
\begin{center}
\fbox{\includegraphics[width=7cm]{mb-03.png}}
\end{center}
\caption{Paralelismo en Conjuntos de {\it Mandelbrot}}
\label{fig:par3}
\end{figure}

\item Cerca del 50\% del tiempo se invierte en una única línea de código, confirmado por la Figura \ref{fig:bot3}.
  
\begin{figure}[H]
\begin{center}
\fbox{\includegraphics[width=7cm]{mb-04.png}}
\end{center}
\caption{Cuellos de Botellas en Conjuntos de {\it Mandelbrot}}
\label{fig:bot3}
\end{figure}

\item Algunos ciclos ya se encuentran optimizados, se identifica el uso de instrucciones vectoriales como {\tt movss, addss} en la Figura \ref{fig:vec3}.

\begin{figure}[H]
\begin{center}
\fbox{\includegraphics[width=7cm]{mb-05.png}}
\end{center}
\caption{Vectorización en Conjuntos de {\it Mandelbrot}}
\label{fig:vec3}
\end{figure}
...
\includepdf[frame=true, pages=-, scale=0.75, pagecommand={}]{hotspot-paper.pdf}
