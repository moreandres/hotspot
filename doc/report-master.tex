...
La infraestructura informó correctamente que el sistema corre un núcleo {\it Linux} versión 3.16.0 en un sistema {\it Ubuntu} versión 14.04, incluyendo un compilador {\it GCC} versión 4.8.2 y una librería {\it GNU C} versión 2.19. La identificación del {\it software} utilizado en el sistema permite ser más específicos al comparar los resultados obtenidos.

\begin{center}
\fbox{\includegraphics[width=8cm]{sp-01.png}}
\end{center}

La infrastructura ejecutó exitosamente pruebas de rendimiento donde entre otras se incluyen {\it DGEMM} (con 1.8 mflops) y {\it STREAM} (con 7 MB/s). Estos datos sirven como límite de capacidad del sistema en la práctica ya que son núcleos de cómputo denso diseñados para obtener rendimiento en bruto.
...
\item En la Figura \ref{fig:par} la infrastructura estima un 85\% de paralelismo, con una mejora teórica máxima de 7 veces sin escalar el tamaño del problema si utilizamos el mismo algoritmo.
...
\item No hay lecturas ni escrituras al disco, por lo que no se está perdiendo rendimiento al utilizar memoria virtual.
...
Esto implica que los ciclos de cómputos del algoritmo no están siendo vectorizados, desaprovechando la capacidad de cómputo disponible. Una comparación de rendimiento de diferentes algoritmos de multiplicación de matrices puede encontrarse en \cite{mm-tool}.
...
El reporte generado por la infraestructura puede consultarse en el apéndice \ref{example-heat2d}. Algunas conclusiones que pueden obtenerse son:
...
\item El paralelismo en el programa solo alcanza el 65\%, por lo tanto el límite máximo de mejora es de 3 veces.

\begin{center}
\fbox{\includegraphics[width=10cm]{he-06.png}}
\end{center}
  
\item Hay dos cuellos de botella con un 25\% y un 15\% del tiempo de ejecución.

\begin{center}
\fbox{\includegraphics[width=10cm]{he-07.png}}
\end{center}

\item La utilización del CPU no es uniforme.
...
El reporte generado puede consultarse en el apéndice \ref{example-mandel}. Algunas conclusiones que pueden obtenerse al ejecutar la infraestructura desarrollada sobre una implementación básica son:
